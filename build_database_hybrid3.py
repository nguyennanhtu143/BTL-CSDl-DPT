...
from src.database_hybrid3 import (
    connect,
    count_images,
    existing_filenames,
    extract_hybrid3_from_path,
    init_db,
    read_image_meta,
    reset_db,
    upsert_image,
...
def build(dataset_dir: Path, db_path: Path, rebuild: bool = False, limit: int | None = None) -> None:
    ...
    if rebuild:
        print(f"[build-hybrid3] --rebuild: xoá CSDL cũ tại {db_path}")
        reset_db(db_path)
    else:
        init_db(db_path)
    ...
    print(f"[build-hybrid3] Dataset   : {dataset_dir}")
    print(f"[build-hybrid3] CSDL      : {db_path}")
    print(f"[build-hybrid3] Tổng ảnh   : {len(files)}")
    print(f"[build-hybrid3] Đã có     : {len(skip)} (skip)")
    print(f"[build-hybrid3] Cần xử lý : {len(to_process)}")
    print("-" * 60)

    if not to_process:
        print("[build-hybrid3] Không có ảnh mới. Done.")
        return
    ...
    with connect(db_path) as conn:
        for i, p in enumerate(to_process, start=1):
            ...
            try:
                ...
                hist, grad, compact, scalars = extract_hybrid3_from_path(p)
                upsert_image(
                    conn=conn,
                    filename=p.name,
                    width=w,
    ...
    print("-" * 60)
    print(f"[build-hybrid3] Hoàn tất xử lý {len(to_process)} ảnh trong {time.perf_counter()-t_start:.2f}s")
    if fails:
        print(f"[build-hybrid3] {len(fails)} ảnh lỗi")
    print(f"[build-hybrid3] Tổng record: {count_images(db_path)}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build CSDL hybrid-3 (color + gradient + compact6)")
